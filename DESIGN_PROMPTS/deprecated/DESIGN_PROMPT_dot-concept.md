...
| **v2 (현재)** | 2026-08-18 | **피드백 반영**: "보라색 포인트가 취향이 약하다" → 포인트 컬러를 **검정**으로 전면 교체, **바이닐 레코드(원반+그루브)** 모티프로 재설계해 시각적 존재감 강화 |
...
- 보라/파랑 계열(primary, accent)은 도트 컨셉 그래픽 자체에는 절대 사용하지 않는다
  (버튼 hover 등 브랜드 요소가 아닌, 순수 흑백 대비로만 구성)
...
1. DotEmptyState — 갤러리/탐색/피드/시리즈 빈 상태 공용 컴포넌트
   - **바이닐 레코드 그래픽**: 원반(disc, 반지름 66px) + 그루브 도트 6개 링(바깥 링일수록 도트
     촘촘하고 큼, 링마다 각도 엇갈림) + 중앙 라벨 원(반지름 22px) + 이모지 아이콘을 라벨 위에 오버레이
   ...
   - **light 테마**: 검정(#0a0a0a) 원반 + 흰색(#f5f5f7) 그루브 도트/라벨로 반전 — 밝은 페이지
     배경과 강한 대비. 양쪽 테마 모두 "포인트 컬러 = 검정" 원칙을 유지한다(반전은 배경 대비를 위한 것)
...
| 항목 | v1 (보라 산포) | v2 (레코드) |
|------|---------------|------------|
| 형태 | 15×11 격자에 흩뿌린 점 | 하나의 응집된 원형 오브젝트(레코드판) |
| 색상 | primary/accent 보라·파랑 교대 | 검정×흰색 고대비 (테마 반전) |
| 임팩트 | 배경에 녹아들어 존재감 약함 | 중심축이 명확해 시선을 즉시 붙잡음 |
| 브랜드 컬러 의존 | 있음(기존 UI 톤과 중복돼 밋밋) | 없음(도트 컨셉만의 고유 아이덴티티) |
| "레코드" 컨셉 부합 | 없음(추상적 산포) | 원반+그루브+라벨 구조로 직접 표현 |

**결론**: 브랜드 컬러(보라/파랑)는 버튼·탭 등 기존 UI 전역에서 이미 충분히 쓰이고 있어, 도트 컨셉까지 같은 팔레트를 쓰면 차별점이 사라진다. 도트 컨셉만큼은 흑백 고대비로 완전히 분리해 "이 앱에만 있는 특별한 순간(로딩·빈 상태)"이라는 인상을 준다.
...
│              ⚪ ⚫ ⚪ ⚫ ⚪ ⚫ ⚪            │
│                 ⚫ ⚪ ⚫ ⚪ ⚫               │
│            팔로우한 작가가 없어요              │  17px 800 (라이트 text 컬러)
│                                             │
│           ┌────────────────────┐            │  버튼: 검정 배경 + 흰 텍스트
│           │    작가 탐색하기      │            │
│           └────────────────────┘            │
...
- 도트 컨셉 그래픽 자체에는 `COLORS.primary`/`COLORS.accent`(보라·파랑) 사용 금지 — 순수 흑백만
...
- [x] AC4: 도트 컨셉 그래픽에 보라/파랑(primary/accent) 미사용, 순수 흑백 고대비만 사용
- [x] AC5: 순수 JSX SVG, 외부 에셋/라이브러리 없음
- [x] AC6: 모바일/데스크탑 레이아웃 정상(고정 크기 SVG라 컨테이너보다 항상 작음)

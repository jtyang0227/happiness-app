...
│   ├── BookingScreen.js  # (신규, Feature 38-B4) 예약 조회 — 상태별(대기중/확정됨/완료/취소·거절) SectionList,
│   │                       REQUESTED 건에 확인/거절 Alert 액션. 가용 시간 설정은 웹 `/bookings`로 Linking 딥링크 안내
│   │                       (모바일 UI로 만들지 않음 — 복잡도상 P3 보류). `ProfileScreen` 메뉴 "📅 예약 관리"에서 진입.
...
│   │   ├── bookingApi.js    # (신규, Feature 38-B4) getMyBookings/confirmBooking/rejectBooking — 웹 bookingApi.js의 부분집합(가용시간 설정 없음)

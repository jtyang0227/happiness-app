...
2. **가정(Assumption)을 명시한 후 진행** — 구현 전 가정한 내용을 한 줄로 밝히고 바로 진행한다. 여러
   해석이 가능하면 가장 가능성 높은 해석을 고르고 무엇을 선택했는지 밝힌다. 더 단순한 대안이 있다면
   질문으로 작업을 막지 말고, 진행하면서 함께 언급한다. 사용자에게 되묻는 것은 "무엇을 구현할지" 자체가
   바뀌고 잘못된 선택을 저렴하게 되돌릴 수 없는 경우로 한정한다 — "어떻게 구현할지" 수준의 문제는 스스로
   판단하고 진행한다(아래 12번 "모호한 요구사항 처리 우선순위"와 함께 적용). 한 작업 안에서 두 번째
   질문이 필요해진다면 우선순위 판단 자체를 다시 점검한다.
3. **Plan 먼저** — Goal / Constraints / Existing Context / Implementation Plan / Verification Plan을
   내부적으로 정리한 뒤 구현을 시작한다. 복잡한 작업은 여러 단계로 분해한다.
4. **단순함 우선** — 문제를 해결하는 최소한의 코드만 작성한다. 요청받지 않은 기능·추상화·"유연성"·
   "설정 가능성"을 넣지 않고, 일어날 수 없는 시나리오에 대한 에러 처리를 만들지 않는다. 일회성 코드에
   추상화를 씌우지 않는다. "시니어 엔지니어가 보면 과하다고 할까?"를 스스로 묻고, 그렇다면 단순화한다.
5. **작은 단위로 실행** — 탐색 → 최소 변경 → 검증 → 확인 → 다음 변경 순으로 진행하고, 각 단계가 이전
   단계의 가정을 깨지 않는지 확인한다.
6. **기존 코드를 최대한 존중하고, 정밀하게 수정** — 기존 함수·컴포넌트·유틸리티·에러 처리 방식·
   네이밍·디렉터리 구조·기존 dependency를 우선 재사용한다. 구현 가능하다는 이유만으로 새 라이브러리를
   추가하지 않는다. 수정 시 반드시 고쳐야 하는 부분만 건드린다 — 인접 코드·주석·포맷팅을 임의로
   "개선"하지 않고, 망가지지 않은 코드는 리팩터링하지 않으며, 마음에 들지 않아도 기존 스타일을 따른다.
   관련 없는 죽은 코드를 발견하면 언급만 하고 삭제하지 않는다. 단, 자신의 수정으로 더 이상 쓰이지 않게
   된 import·변수·함수는 제거한다. 기준: 변경된 모든 줄이 사용자의 요청과 직접 연결되어야 한다.
7. **검증은 선택이 아니다** — 이 저장소에 실제로 존재하는 명령만 사용한다(다른 스택의 `pytest`/`go test`
   ...
   실행한 정확한 명령과 결과("통과"/"X로 실패"/"Y라서 미실행")를 있는 그대로 보고하고, 구체적인 검증
   근거 없이 "완료"·"고쳤다"·"동작한다"라고 말하지 않는다. 사용자가 "끝났어?"라고 묻기 전에 먼저
   실행한다.
8. **실패하면 원인을 분석하고 수정한다** — 실패 상태로 완료라고 보고하지 않는다. 확인 → 원인 분석 →
   관련 코드 확인 → 수정 → 재실행 → 통과 확인 루프를 돈다. 에러 은닉, 테스트 삭제, 검증 우회로 실패를
   숨기지 않는다.
9. **자기검토(Self Review)** — 완료 전 Correctness / Regression / Edge Cases / Security / Performance /
   Maintainability / Scope(요청 범위를 벗어난 불필요한 변경 여부) 7가지를 스스로 점검한다.
10. **git diff 확인** — push 전 `git status`/`git diff`로 의도치 않은 파일 변경, 디버그 코드
    (console.log 등), 시크릿·크레덴셜, 임시 파일이 섞이지 않았는지 확인한다.
11. **요구사항 대조** — 최초 요구사항을 항목별로 완료/미완료 대조하고, 미완료 항목이 있으면 계속
    진행한다.
12. **모호한 요구사항 처리 우선순위** — 기존 코드 패턴 → 프로젝트 문서(CLAUDE.md) → 테스트 코드 →
    설정 파일 → 사용자의 명시적 요구사항 → 일반적인 소프트웨어 엔지니어링 관행 순으로 판단한다. 이래도
    판단 불가능한 중요한 결정만 사용자에게 질문하고, 사소한 구현 디테일 때문에 작업을 중단하지 않는다.
13. **위험한 변경은 먼저 확인** — DB 스키마 변경, 데이터 삭제, 인증/권한 변경, 운영 설정 변경, 마이그
    레이션, 대규모 삭제, 배포 설정 변경은 영향을 먼저 분석한다(운영 DB `ddl-auto: validate` 규칙과 연계).
14. **절대 금지** — 테스트를 무력화하거나 삭제해서 통과시키기, 에러 은닉용 try-catch 남발, lint 규칙
...
않은 항목은 통과했다고 표현하지 않는다. 변경이 사소하지 않다면 마지막에 이번 작업에서 실제로 중요했던
개념·트레이드오프·함정(gotcha) 하나만 2~3문장으로 짧게 덧붙인다 — 왜 뻔한 방식 대신 이 방식을 택했는지,
어떤 기본값을 신뢰했는지, 규모가 커지면 무엇이 먼저 깨지는지 같은, 코드 자체가 보여주지 않는 내용이어야
한다. diff를 그대로 되풀이하는 수준이거나 사용자가 이미 아는 내용이면 생략한다.
...
- **Feature 39(b)(c) — 촬영 준비 체크리스트 & 수금 현황**: `DESIGN_PROMPTS/design/DESIGN_PROMPT_booking-checklist-payment.md`. `Booking` 엔티티에 8개 컬럼 추가 — `checklistJson`(TEXT, `[{"id","text","checked"}, ...]` JSON 배열), `deliveryDeadline`(DATE), `depositStatus`/`balanceStatus`(VARCHAR 20, PENDING/RECEIVED, 기본 PENDING) + `depositAmount`/`balanceAmount`(INTEGER, 옵션) + `depositReceivedAt`/`balanceReceivedAt`(TIMESTAMP, RECEIVED 전환 시 자동 기록·PENDING로 되돌리면 null). `PUT /api/booking/{id}/checklist`(`BookingService.updateChecklist`) / `PUT /api/booking/{id}/payment`(`BookingService.updatePayment`, null 필드는 변경하지 않는 부분 업데이트 — 토글 클릭마다 상태 하나만 보내도 되고 금액은 별도로 저장 가능) 신규 엔드포인트, 둘 다 `findByIdAndMemberId` IDOR 검사. **components/booking/ChecklistAccordion.jsx** — 기본 접힘 아코디언(완료 n/m 헤더), 텍스트 입력으로 항목 추가, 체크박스 완료 표시, 항목별 삭제, 납품기한 date input — 모든 조작이 즉시 서버 자동저장. `isDeliveryDeadlineNear()`(export) — 오늘 기준 0~3일 이내면 true, 컴팩트 리스트·풀 카드·상세 패널 3곳 모두에서 주황색 "🚚 납품 임박" 배지로 재사용. **components/booking/PaymentToggle.jsx** — 계약금/잔금 각각 독립 토글(미수령 회색 ↔ 수령완료 초록톤), 금액은 선택 입력(AC-c5, 토글은 금액 없이도 동작). **BookingDashboard.jsx** — 상단 탭에 "미수금" 추가(`CONFIRMED` 예약 중 `depositStatus`/`balanceStatus` 어느 한쪽이라도 PENDING인 것만 클라이언트 필터, 이 탭에서 수령완료로 토글하면 즉시 목록에서 사라짐), CONFIRMED 카드(컴팩트·풀·데스크탑 상세 패널 3곳)에 체크리스트·수금 섹션 추가. 월간 수금 요약 카드(`GET /api/booking/payment-summary`)는 P2로 이번 스코프에서 제외(PLAN_39 AC-c3).

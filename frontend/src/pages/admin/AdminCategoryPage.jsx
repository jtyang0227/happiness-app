    ...
    <AdminLayout currentPageTitle="카테고리 관리">
      <div style={{ maxWidth: 900 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 14, marginBottom: 24 }}>
          {[
            { label: '전체 사진',      value: total,          icon: '📷', color: '#3182F6' },
            { label: '분류 완료',      value: genreTotal,     icon: '✅', color: '#43a047' },
            { label: '미분류',        value: uncategorized,  icon: '⚠️', color: '#f59e0b' },
            { label: '사용 장르 수',  value: sortedStats.filter(s => s.count > 0).length, icon: '🎨', color: '#4E9FFF' },
          ].map(card => (
          ...
          <div style={{
            ...
            <div>
              ...
              <div style={{ fontSize: 12, color: '#9090b0', marginTop: 2 }}>
                <a href="/admin/photos" style={{ color: '#3182F6', fontWeight: 600 }}>사진 관리</a>
        ...
        <div style={{
          ...
          • <a href="/admin/photos" style={{ color: '#3182F6' }}>사진 관리</a>에서 각 사진의 장르를 개별 수정할 수 있습니다.
        </div>
      </div>
    </AdminLayout>
  );

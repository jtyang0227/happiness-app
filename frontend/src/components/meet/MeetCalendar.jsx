...
import { COLORS as TOKENS } from '../../constants/colors';

const COLORS = {
  mine: '#3182F6',
  theirs: '#4E9FFF',
  overlap: '#10b981',
  past: TOKENS.surfaceDim,
  today: '#1B64DA',
};
  ...
  return (
    <div style={{ userSelect: 'none' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
        ...
        <span style={{ color: TOKENS.text, fontWeight: 600, fontSize: 15 }}>{monthName}</span>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 2, marginBottom: 4 }}>
        {DAY_LABELS.map((d, i) => (
          <div key={d} style={{ textAlign: 'center', fontSize: 11, color: i === 0 ? '#e0455a' : i === 6 ? '#3182F6' : TOKENS.textMuted, padding: '2px 0' }}>
            {d}
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 2 }}>
        {cells.map((day, idx) => {
          ...
          let bg = 'transparent';
          let color = isPast ? TOKENS.textHint : TOKENS.textSecondary;
          ...
          return (
            <div
              ...
              style={{
                ...
                boxShadow: isSelected ? `0 0 0 2px ${TOKENS.primaryLight}` : 'none',
              }}
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
              ...
              <span style={{ color: TOKENS.text, fontWeight: 700, fontSize: 14 }}>{formatDateKo(memoDate)}</span>
            </div>
            {isConfirmedDay && (
              <span style={{ fontSize: 11, background: TOKENS.successTonal, color: TOKENS.success, borderRadius: 20, padding: '2px 9px', fontWeight: 600 }}>
                ✅ 확정일
              </span>
            )}
            <button
              onClick={() => setSelectedDate(null)}
              style={{ background: 'none', border: 'none', color: TOKENS.textSecondary, fontSize: 16, cursor: 'pointer', padding: 0, lineHeight: 1 }}
            >
              ✕
            </button>
          </div>
          ...
          <div style={{ marginBottom: 12 }}>
            <div style={{ fontSize: 11, color: TOKENS.textMuted, fontWeight: 600, letterSpacing: '0.05em', textTransform: 'uppercase', marginBottom: 7 }}>
              참여 인원
            </div>
          ...
          {(meetInfo?.locationName || meetInfo?.locationAddress) ? (
            <div style={{ borderTop: `1px solid ${TOKENS.border}`, paddingTop: 11 }}>
              <div style={{ fontSize: 11, color: TOKENS.textMuted, fontWeight: 600, letterSpacing: '0.05em', textTransform: 'uppercase', marginBottom: 7 }}>
                약속 장소
              </div>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
                ...
                <div>
                  {meetInfo.locationName && (
                    <div style={{ color: TOKENS.text, fontSize: 13, fontWeight: 600 }}>{meetInfo.locationName}</div>
                  )}
                  {meetInfo.locationAddress && (
                    <div style={{ color: TOKENS.textMuted, fontSize: 12, marginTop: 2 }}>{meetInfo.locationAddress}</div>
                  )}
                </div>
              </div>
            </div>
            ...
            <div style={{ borderTop: `1px solid ${TOKENS.border}`, paddingTop: 11 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 7, color: TOKENS.textSecondary, fontSize: 12 }}>
                <span>📍</span>
                <span>장소 미정</span>
              </div>
            </div>
          ...
          {isConfirmedDay && meetInfo?.confirmedTime && (
            <div style={{ borderTop: `1px solid ${TOKENS.border}`, paddingTop: 11, marginTop: 11 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                <span style={{ fontSize: 14 }}>⏰</span>
                <span style={{ color: TOKENS.success, fontSize: 13, fontWeight: 600 }}>{meetInfo.confirmedTime}</span>
              </div>
            </div>
          )}
          ...
          {memoHasMine && memoHasTheirs && (
            <div style={{
              marginTop: 11, borderTop: `1px solid ${TOKENS.successTonal}`, paddingTop: 11,
              background: TOKENS.successTonal, borderRadius: 8, padding: '8px 12px', marginLeft: -4, marginRight: -4,
            }}>
              <div style={{ color: TOKENS.success, fontSize: 12, fontWeight: 600 }}>
                ✓ 양측 모두 가능한 날입니다
              </div>
            </div>
          )}
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
      <div style={{
        ...
        background: isMe ? TOKENS.primaryLight : '#EAF3FF',
      ...
      <div style={{ flex: 1 }}>
        <span style={{ color: TOKENS.text, fontSize: 13 }}>{name}</span>
        {isMe && <span style={{ color: TOKENS.textMuted, fontSize: 11, marginLeft: 5 }}>(나)</span>}
      </div>
      <span style={{
        fontSize: 11, padding: '2px 9px', borderRadius: 20, fontWeight: 600,
        background: available ? TOKENS.successTonal : TOKENS.surfaceDim,
        color: available ? TOKENS.success : TOKENS.textMuted,
      }}>
        {available ? '✓ 가능' : '미표시'}
      </span>
    </div>
